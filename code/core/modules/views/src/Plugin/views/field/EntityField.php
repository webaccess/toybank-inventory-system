...
class EntityField extends FieldPluginBase implements CacheableDependencyInterface, MultiItemsFieldHandlerInterface, DependentWithRemovalPluginInterface {
  ...
  protected function defineOptions() {
    ...
    // This default mirrors Drupal 7 behavior.
    $options['group_rows_render_together'] = array(
      'default' => FALSE,
    );
  ...
  public function multiple_options_form(&$form, FormStateInterface $form_state) {
    ...
    $form['group_rows_render_together'] = array(
      '#title' => $this->t('Render all items together'),
      '#type' => 'checkbox',
      // Drupal 7 behavior was to render multiple values individually. Use that
      // as the default.
      '#default_value' => $this->options['group_rows_render_together'],
      '#description' => $this->t('If checked, multiple values will be rendered in a single pass. Select this option to have greater control over the display of multi-valued fields. Note: you must use the "Rewrite results" options below to render the individual items within the "-multiple" token.'),
      '#fieldset' => 'multiple_field_settings',
      '#states' => array(
        'visible' => array(
          ':input[name="options[group_rows]"]' => array('checked' => TRUE),
          ':input[name="options[group_rows_render_together]"]' => array('checked' => FALSE),
