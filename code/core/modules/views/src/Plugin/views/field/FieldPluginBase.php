  ...
  public function advancedRender(ResultRow $values) {
    ...
    if ($this->allowAdvancedRender()) {
      ...
      if ($this instanceof MultiItemsFieldHandlerInterface) {
        $prev_alter_setting = FALSE;
        if ($this instanceof Field && $this->options['group_rows_render_together'] && $this->options['alter']['alter_text']) {
          // Prevent rewriting the field until all items in this field have been
          // rendered.
          $prev_alter_setting = TRUE;
          $this->options['alter']['alter_text'] = FALSE;
        }
      ...
      if ($prev_alter_setting) {
          $this->options['alter']['alter_text'] = TRUE;

          // Use a fake item to build render tokens.
          $tokens = $this->getRenderTokens(['alter_text' => TRUE, 'text' => '']);
          // Add the _multiple token, which holds all the items rendered above,
          // but not altered, and alter then all at once.
          $tokens['{{ ' . $this->options['id'] . '_multiple }}'] = $items;
          $value = $this->renderAltered($this->options['alter'], $tokens);
        }
        else {
          $value = $this->renderItems($items);
        }
